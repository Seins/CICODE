...
class AdbCommand(object):
    ...
    def pull(self, fromPath, toPath=""):
        ...
        '''
        if toPath == "":
            cmd = 'adb -s %s pull \"%s\"'%(INFO.DEVICE, fromPath)
        else:
    ...
    def startActivity(self, component=""):
        cmd = 'adb -s %s shell am start -S -n %s'%(INFO.DEVICE, component)
        ...
        while(1):
            ...
            if len(strTemp)==0:
                time.sleep(0.1)
                timeout=timeout-(time.time()-while_time)
                if timeout==0:
                    raise AttributeError('startActivity is timeout')
...
#                self.pull('/data/local/tmp/temp.png', savePath)
            ...
            while os.path.exists(newname):
                ...
                picname = u'%s_%s.png'%(saveName,count)
                count += 1
    ...
    def screenIsLock(self):
        p = self.__runshell('adb -s %s shell dumpsys window|findstr mShowingLockscreen'%(INFO.DEVICE))
        msg = p.stdout.readline()
        if(msg.lstrip(' ').split(' ')[0].split('=')[1] == 'false'):
            return False
        else:
            return True
    ...
    def getDeviceSerial(self):
        ...
        if results.file_path:
            FLAG.FILEPATH = results.file_path
        if results.log_path:
            FLAG.LOGPATH = results.log_path        
        if results.device_serial:
            p = self.__runshell('adb -s %s shell assertid' %(results.device_serial))
            info = p.stdout.read()
            if info.find('assertid: not found')<0:
                raise AttributeError('device %s not found or device offline' %(results.device_serial))
            ...
            for i in xrange(deviceLen):
                if infoList[i].find('device') >= 0:
                    ...
                    break
            if INFO.DEVICE == None:
                raise AttributeError('devices offline')
    ...
    def getDeviceName(self):
        ...
        INFO.DEVICENAME = INFO.DEVICENAME.decode('utf-8')
    ...
    def getLogPath(self,scriptPath):
        ''' 
        '''
        logPath = '%s_%s'%(scriptPath.decode('gbk').encode('utf-8'), INFO.DEVICENAME)
        return logPath
