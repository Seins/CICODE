...
from automatormonkey.monkeyrunnercore.AndroidManifest import AndroidManifest
...
class rMonkeyRunner(object) :
    ...
    def getApkPackageName(self, path):
        return self.__androidManifast.getPackage(path)
    ...
    def screenIsLock(self):
        return self.device.screenIsLock()
    ...
    def __longclickOperate(self,TAG,value, match):
        operator = {
                    UIELEMENT.CLASSNAME:lambda:self.__click.longByClass(value, match),
                    UIELEMENT.TEXT:lambda:self.__click.longByText(value, match),
                    }
        operator[TAG]() 
    ...
    def __wakeUpAndUnlock(self):
        if self.screenIsLock():
