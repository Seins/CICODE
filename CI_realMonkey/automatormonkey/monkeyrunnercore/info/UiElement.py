...
class UiElement(object):
    ...
    def equal(self, destElement):
        nodeInfo = str(self.element)
        if nodeInfo.find('Comment')>0:
            return False
        if len(self.element.attributes.keys())!=len(destElement.getElement().attributes.keys()):
            return False
        else :
            for key in self.element.attributes.keys():
                if self.element.getAttribute(key)!=destElement.getElement().getAttribute(key):
                    return False
        return True        
